...
import { createPolicyRegistry, definePolicy } from "../src/gateway/policy.js";
...
function createMockDb() {
  ...
  function makeChain(rows: typeof mockRows) {
    ...
    chain.values = vi.fn(() => ({
      returning: vi.fn().mockResolvedValue(rows.slice(0, 1)),
    }));
    chain.set = vi.fn(() => {
      const sub: any = {};
      sub.where = vi.fn(() => ({
...
describe("Gateway Handler", () => {
  ...
  beforeEach(() => {
    ...
    app.use(
      "/api/gateway",
      createGatewayHandler({ db: mockDb as any, policies }),
    );
  });
  ...
  it("should reject invalid request format", async () => {
    const res = await request(app).post("/api/gateway").send({ garbage: true });
  ...
  it("should prevent client from overriding tenantId filter", async () => {
    const res = await request(app)
      ...
      .send({
        table: "contacts",
        operation: "findMany",
        payload: { where: { tenantId: "attacker-tenant" } },
      });
    expect(res.status).toBe(403);
    expect(res.body.error).toContain("Disallowed filters");
  });
...
describe("Gateway Batch Handler", () => {
  ...
  it("should call onError callback on failure", async () => {
    ...
    errorApp.use(
      "/api/gateway",
      createGatewayHandler({ db: errorDb as any, policies, onError }),
    );
