...
} from "../src/gateway/auto-policy.js";
...
describe("definePolicyFromSchema", () => {
  it("should auto-expose all columns from a table", () => {
    ...
    expect(policy.allowedColumns).toEqual([
      "id",
      "tenantId",
      "name",
      "industry",
...
describe("definePoliciesFromSchema", () => {
  it("should generate policies for all tables in schema", () => {
    ...
    expect(policies.map((p) => p.tableName).sort()).toEqual([
      "accounts",
      "contacts",
    ]);
  });
  ...
  it("should skip non-table exports", () => {
    const policies = definePoliciesFromSchema({
      accounts,
      contacts,
      someHelper: () => "nope",
      someStr: "nope",
  ...
  it("should apply per-table overrides", () => {
    ...
    expect(
      policies.find((p) => p.tableName === "contacts")?.allowedColumns,
    ).not.toContain("notes");
    expect(
      policies.find((p) => p.tableName === "accounts")?.allowedColumns,
    ).toContain("name");
  });
