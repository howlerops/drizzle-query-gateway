...
describe("definePolicy", () => {
  ...
  it("should preserve all config fields", () => {
    expect(contactsPolicy.allowedFilters).toEqual([
      "status",
      "ownerId",
      "createdAt",
    ]);
    expect(contactsPolicy.allowedColumns).toEqual([
      "id",
      "name",
      "email",
      "status",
    ]);
    expect(contactsPolicy.requiredFilters(mockCtx)).toEqual({
      tenantId: "tenant-1",
    });
