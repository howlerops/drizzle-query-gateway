 ...
 */
export function definePolicy<T extends Table>(
...
export function validateShape(
  ...
  if (payload.where) {
    ...
    const disallowed = clientFilterKeys.filter(
      (k) => !policy.allowedFilters.includes(k),
    );
  ...
  if (payload.columns) {
    const disallowedCols = payload.columns.filter(
  ...
  if (payload.data && isWrite) {
    ...
    const disallowedData = dataKeys.filter(
      (k) => !policy.allowedColumns.includes(k),
    );
